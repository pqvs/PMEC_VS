...
FuzzyVE::FuzzyVE(std::string file_name){

	this->veRules=std::shared_ptr<VERules>(new VERules(file_name));
...
float FuzzyVE::defuzzyfier(std::vector<float> &inputs) {
	...
	return this->veRules->fuzzy(&inputs[0], this->input_Variables,this->output_variable);
}