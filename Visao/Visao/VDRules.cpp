...
VDRules::VDRules(std::string file_name)
{
...
VDRules::VDRules():baseRuleMatriz(nullptr){


}

void VDRules::generate_baseRuleMatrix(std::string file_name){
	//Read from file_name
...
float VDRules::fuzzy(float* inputs, std::vector<std::shared_ptr<FuzzyFunction>> &input_variables,FuzzyFunction *output_variable){
...
void VDRules::printBaseRuleMatriz() {
	for (int i = 0; i < this->Angulo; i++) {
		for (int j = 0; j < this->Distancia; j++) {
